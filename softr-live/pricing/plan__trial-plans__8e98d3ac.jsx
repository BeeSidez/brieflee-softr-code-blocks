// Plan page — PICK A PLAN block (design lifted from /get-started).
// Visible to a logged-in user with no active Stripe subscription.
//
// Annual billing only, with a 30-day 100% money-back guarantee. There is no
// trial. This block used to read the six *_trial Payment Link formulas off
// the users table and render a monthly/yearly toggle; both are gone.
//
// The three annual Payment Links are constants below. Every one of them
// redirects to /payment-success?session_id=…, which branches on whether the
// buyer is logged in. Anyone arriving from here already is, so they land on
// the thank-you and the onboarding booking.
//
// The users read exists only to prefill the checkout email.
...
// Annual only. `reads` is the monthly-equivalent headline, `price` is what
// Stripe charges once a year, `perReview` is price ÷ (maxVideos × 12). The
// per-review figure falls 0.33 → 0.25 → 0.20 up the ladder, which is the
// clearest argument for buying up, so it belongs on the card.
const PLANS = [
  {
...
const TRUST_ITEMS = [
  { iconUrl: TRUST_ICONS.moneyBack, title: "30 days, 100% back", body: "Not saved you time? Take all of it back." },
  { iconUrl: TRUST_ICONS.daysFree, title: "500 bonus reviews", body: "Turn up to onboarding, 100 a month for five." },
  { iconUrl: TRUST_ICONS.stripeSecure, title: "Stripe-secure", body: "We never store card details." },
  { iconUrl: TRUST_ICONS.instantSetup, title: "Instant access", body: "Reviewing in under 5 minutes." },
];
...
export default function Block() {
  ...
  const userEmail = f.email || f.PBrIP || "";
  ...
  /* Referly attributes a referred purchase through client_reference_id. The
     ...
     writes that parameter, so it is free. No affiliate, no change to the URL. */
  const withAffiliate = (url) => {
    const id = typeof window !== "undefined" ? window.affiliateId : null;
    return id ? addParam(url, "client_reference_id", id) : url;
  };

  const handleContinue = () => {
    if (!selectedPlan) return;
    let url = CHECKOUT_URL[selectedPlan.tier];
    if (!url) return;
    url = addParam(url, "prefilled_email", userEmail);
    ...
    <div className="gs-root">
      ...
      <div className="gs-page">
        <div className="text-center mb-6">
          ...
          <h1 className="gs-headline">Pick your plan</h1>
          <p className="gs-sub">Every plan carries the whole toolkit. Billed annually, and if Brieflee has not saved you time in thirty days you get all of it back.</p>
        </div>
        ...
        <p className="text-center mt-6" style={{ fontSize: 13, color: "#555" }}>
          Above 2,000 reviews a month is an enquiry, priced to the programme.{" "}
          <a href="/book-a-demo" style={{ color: "#294ff6", fontWeight: 600 }}>Talk to us</a>.
        </p>
        ...
        <div className="sticky-cta">
          <div className="row gap-3">
            {selectedPlan ? (
              ...
              <div style={{ fontSize: 13, color: "#555" }}>Pick a plan to continue · 30 days, 100% back</div>
            )}
          </div>
