...
|---|---|---|
| `Onboarding` | On user signup (BL \| New Signup workflow) | When they pay (BL \| New Stripe Sub) or hit day 97 dormant |
| `Subscribers` | On Stripe subscription creation | On cancellation |
| `Lapsed` | On subscription cancellation, payment failure beyond grace, dormant deletion | When/if they convert again |

The `Onboarding` audience drives the 14-day Contact-trigger automation set up in EmailIt UI. Adding someone to this audience kicks off the drip automatically.
...
- **Drip** (`bl-features-day-XX`) — 14 educational emails, one per day for 14 days, sent automatically by EmailIt when user is in `Onboarding` audience
...
5. **If drip:** add to the EmailIt automation that watches `Onboarding` audience
...
- **User getting drip emails after they paid** — the `BL | New Stripe Sub` workflow forgot to remove from `Onboarding` audience. Fix the workflow.
- **Drip not starting** — check user was actually added to `Onboarding` audience (look in EmailIt subscribers list)
