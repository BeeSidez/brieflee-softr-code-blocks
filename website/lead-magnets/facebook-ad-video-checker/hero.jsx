...
export default function Block() {
  ...
  const handleSubmitEmail = async () => {
    ...
    try {
      ...
      toast.success("Your Facebook ad breakdown is on its way", {
        description: "Check your inbox in a moment.",
      });

      const target = `${SIGNUP_URL}?email=${encodeURIComponent(email.trim())}&intent=facebook-ad-checker`;
          ...
          <div className="grid lg:grid-cols-[1.1fr_1fr] gap-10 lg:gap-14 items-center">
            <div className="space-y-6">
              <div className="inline-flex items-center gap-2 rounded-full border border-border bg-card/70 backdrop-blur pl-1.5 pr-3 py-1 text-xs font-medium text-muted-foreground">
                ...
                Free AI Facebook ad video checker
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight leading-[1.1] text-primary" aria-live="polite">
                  <span>{typedWord}</span>
              ...
              <p className="text-base md:text-lg text-muted-foreground max-w-xl leading-relaxed">
                ...
                structure, pacing, CTA, all scored. Free, no credit card.
              </p>
              ...
              {step === "email" && (
                <div className="space-y-5 pt-2 max-w-md rounded-2xl border border-border bg-card p-6 shadow-[0_20px_60px_-20px_hsl(var(--primary)/0.4)]">
                  <div>
                    ...
                    <h2 className="text-2xl font-bold text-foreground mb-1">Your Facebook ad check is ready</h2>
                    <p className="text-sm text-muted-foreground">
                      Drop your email and we'll send the full report. Hooks, structure, audio, CTA, all scored.
                    </p>
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    <Button onClick={handleSubmitEmail} disabled={submitting || !createRecord.enabled} className="h-12 text-base font-semibold">
                      {submitting ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving</>) : (<>View my check<ArrowRight className="ml-2 h-4 w-4" /></>)}
                    </Button>
                  </div>
                  {error && <p className="text-sm text-destructive" role="alert">{error}</p>}
                  <p className="text-xs text-muted-foreground">
                    Already have an account? <a href={LOGIN_URL} className="underline font-medium text-foreground">Log in</a>
                  </p>
                </div>
              )}
            </div>
