// =====================================================================
// video-breakdown — the whole page as ONE block (v2 holding version).
//
...
import { ArrowRight, Check, Upload, Link as LinkIcon, Loader2, Mic2, Target, Repeat, Eye, Activity, Sparkles as SparklesIcon, Cpu, FileText, X, Minus, ShoppingBag, Building2, Briefcase, Camera, Plus, ArrowUp } from "lucide-react";
...
const ds = datasource.define({
  leadEvents: "1ca0c0a4-9e53-4eec-b2e5-946093f07ec7", // Brieflee CRM → lead_events
});
...
const OPT_SOURCE  = { id: "0fedec7f-8c6c-447d-8c54-1b4794068602", label: "video-breakdown" };
...
function HeroSection() {
  ...
  // The home hero hands over with ?url= and ?from=. Read inside the
  // useState initialiser on purpose: URL parsing at the top of a block
  // that also creates records stops Softr's analyzer from saving it.
  const [url, setUrl] = useState(() => {
    try { return new URLSearchParams(window.location.search).get("url") || ""; }
    catch (e) { return ""; }
  });
    ...
    try {
      ...
      createEvent.mutateAsync({
        ...
        payload: JSON.stringify({
          ...
          // Where the visitor started, when they were handed over.
          came_from: cameFrom,
        }),
      }).catch((e) => console.error("lead_event write failed (continuing):", e));

      const target = `${DEMO_URL}?email=${encodeURIComponent(email.trim())}&intent=video-breakdown`;
                ...
                <div className="space-y-5 pt-2 max-w-md rounded-2xl border border-border bg-card p-6 shadow-[0_20px_60px_-20px_hsl(var(--primary)/0.4)]">
                  <div>
                    ...
                    <h2 className="text-2xl font-bold text-foreground mb-1">
                      Your video breakdown runs live
                    </h2>
