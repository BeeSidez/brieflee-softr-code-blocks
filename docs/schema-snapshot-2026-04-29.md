...
> Snapshot taken after subscriptions-table migration. New table `subscriptions`, new field `accounts.current_subscription`, and 4 rewired formulas in `usage` (now read from `current_subscription → plan` instead of `billing → pricing`).
...
| `pricing` | `gGyTkXrUpW697X` | 30 |
| `subscriptions` | `s7xluIUfztpBhY` | 17 |
| `billing` | `sQP8T9IIlaJgho` | 41 |
| `usage` | `dw2IeLOtYJLulR` | 30 |
...
- **`Show`** — `CHECKBOX`
- **`subscriptions`** — `LINKED_RECORD` → **subscriptions** _multi_
- **`current_subscription`** — `LINKED_RECORD` → **subscriptions**
- **`subscription_max_videos`** — `LOOKUP` (via `current_subscription` → subscriptions.`max_videos` [LOOKUP]) _readonly_
- **`subscription_max_storage_gb`** — `LOOKUP` (via `current_subscription` → subscriptions.`max_storage_gb` [LOOKUP]) _readonly_
- **`subscription_max_workspaces`** — `LOOKUP` (via `current_subscription` → subscriptions.`max_workspaces` [LOOKUP]) _readonly_
- **`subscription_max_members`** — `LOOKUP` (via `current_subscription` → subscriptions.`max_members` [LOOKUP]) _readonly_
- **`subscription_tier`** — `LOOKUP` (via `current_subscription` → subscriptions.`tier` [LOOKUP]) _readonly_
- **`subscription_status`** — `LOOKUP` (via `current_subscription` → subscriptions.`status` [SELECT]) _readonly_
- **`subscription_period_end`** — `LOOKUP` (via `current_subscription` → subscriptions.`current_period_end` [DATETIME]) _readonly_
- **`subscription_product_name`** — `LOOKUP` (via `current_subscription` → subscriptions.`product_name` [LOOKUP]) _readonly_
- **`subscription_interval`** — `LOOKUP` (via `current_subscription` → subscriptions.`interval` [LOOKUP]) _readonly_
...
- **`creator_email`** — `EMAIL`
- **`submission_open_date`** — `DATETIME` (DATE)
- **`submission_close_date`** — `DATETIME` (DATE)
...
- **`product_screen_time_score`** — `NUMBER`
...
**Table ID:** `s7xluIUfztpBhY`  
**Primary field:** `name`  
**Fields:** 17

- **`name`** — `SINGLE_LINE_TEXT`
- **`account`** — `LINKED_RECORD` → **accounts**
- **`plan`** — `LINKED_RECORD` → **pricing**
- **`stripe_subscription_id`** — `SINGLE_LINE_TEXT`
- **`status`** — `SELECT` [trialing, active, past_due, canceled, paused]
- **`current_period_end`** — `DATETIME` (DATE)
- **`cancel_at_period_end`** — `CHECKBOX`
- **`created_at`** — `CREATED_AT` _readonly_
- **`updated_at`** — `UPDATED_AT` _readonly_
- **`max_videos`** — `LOOKUP` (via `plan` → pricing.`max_videos` [NUMBER]) _readonly_
- **`max_storage_gb`** — `LOOKUP` (via `plan` → pricing.`max_storage_gb` [NUMBER]) _readonly_
- **`max_workspaces`** — `LOOKUP` (via `plan` → pricing.`max_workspaces` [NUMBER]) _readonly_
- **`max_members`** — `LOOKUP` (via `plan` → pricing.`max_users` [NUMBER]) _readonly_
- **`tier`** — `LOOKUP` (via `plan` → pricing.`tier` [SELECT]) _readonly_
- **`interval`** — `LOOKUP` (via `plan` → pricing.`interval` [SELECT]) _readonly_
- **`product_name`** — `LOOKUP` (via `plan` → pricing.`product_name` [SINGLE_LINE_TEXT]) _readonly_
- **`accounts`** — `LINKED_RECORD` → **accounts** _multi_
...
- **`invoice`** — `URL`
- **`stripe_created`** — `DATETIME` (DATETIME)
- **`stripe_cancelled`** — `DATETIME` (DATETIME)
- **`failed_date`** — `DATETIME` (DATETIME)
- **`created_at`** — `CREATED_AT` _readonly_
- **`updated_at`** — `UPDATED_AT` _readonly_
...
- **`max_members`** — `FORMULA` [SINGLE_LINE_TEXT] `SWITCH(
  ...
  "Studio", 10,
 …` _readonly_
- **`max_videos`** — `FORMULA` [SINGLE_LINE_TEXT] `{subscription_max_videos}` _readonly_
- **`created_at`** — `CREATED_AT` _readonly_
- **`updated_at`** — `UPDATED_AT` _readonly_
...
- **`subscription_tier`** — `LOOKUP` (via `accounts` → accounts.`subscription_tier` [LOOKUP]) _readonly, multi_
- **`subscription_max_videos`** — `LOOKUP` (via `accounts` → accounts.`subscription_max_videos` [LOOKUP]) _readonly, multi_
