...
function Engine({ user }) {
  ...
  // Intake state.
  const [tab, setTab] = useState(STEPPER ? "file" : "url");
  ...
  async function runPipeline(item, myRun, ui) {
    ...
    const subFields = {
      ...
      submissionType: context === "submission" ? OPT.subTypeRevision : (briefId ? OPT.subTypeBrief : OPT.subTypeContentReview),
  ...
  const reset = () => { runRef.current += 1; setStep(0); setCtxChoice(""); setPdf(null); setAgents([]); setPage("input"); setError(""); setFailReason(""); setProcSrc(""); setResult(null); setBatch([]); };
  ...
  const savedAgents = useMemo(() => unwrapAll(af?.qaChecklist), [af]);
  const needsQa = STEPPER && !!af && savedAgents.length === 0;
  const order = ["workspace", "upload", "context", ctxChoice ? "detail" : null, needsQa ? "qa" : null].filter(Boolean);
        ...
        <div className="bl-card">
          ...
          {!gate.ok && gate.code !== "loading" ? (
            <div className="bl-gate">
              ...
              {gate.code === "noCredits" || gate.code === "inactive" ? <a className="bl-btn bl-primary" href="/plan">See your plan</a> : null}
            </div>
          ) : null}
          ...
          {gate.ok ? (
            <>
              ...
              {stepKey === "upload" ? (
                <div className="bl-step">
                  ...
                  <div className="bl-grid">
                    <label className="bl-field"><span>Creator name <i>optional</i></span><input value={creatorName} onChange={(e) => setCreatorName(e.target.value)} placeholder="Who made it" /></label>
                    <label className="bl-field"><span>Creator email <i>optional</i></span><input type="email" value={creatorEmail} onChange={(e) => setCreatorEmail(e.target.value)} placeholder="So they get the result" /></label>
                  </div>
                </div>
              ) : null}
              ...
              <div className="bl-actions">
                <span className="bl-meta">{isLast ? `${creditsLeft} video${creditsLeft === 1 ? "" : "s"} left this cycle · ${aiModeLabel} mode` : ""}</span>
                <span className="bl-foot">
                  {step > 0 ? <button type="button" className="bl-btn bl-neutral" onClick={() => { setError(""); goBack(); }}>Back</button> : null}
                  {stepKey === "context" ? <button type="button" className="bl-btn bl-neutral" onClick={() => { setCtxChoice(""); setError(""); if (needsQa) goNext(); else handleStart(); }}>Skip</button> : null}
                  <button
                    type="button"
                    className="bl-btn bl-primary"
                    disabled={(stepKey === "workspace" && !workspaceId) || (stepKey === "upload" && !file) || (stepKey === "context" && !ctxChoice && !isLast) || (stepKey === "qa" && agents.length === 0)}
                    onClick={() => {
                      setError("");
                      ...
                      if (stepKey === "upload" && creatorEmail && !emailOk(creatorEmail)) { setError("That creator email does not look right."); return; }
                  ...
                  >{isLast ? "Review" : "Next"}<ArrowRight size={14} /></button>
                </span>
              </div>
            </>
          ) : null}
        ...
        <div className="bl-card">
          ...
          {!gate.ok && gate.code !== "loading" ? (
            <div className="bl-gate">
              ...
              {!isCreator && (gate.code === "noCredits" || gate.code === "inactive") ? <a className="bl-btn bl-primary" href="/plan">See your plan</a> : null}
            </div>
          ) : null}
