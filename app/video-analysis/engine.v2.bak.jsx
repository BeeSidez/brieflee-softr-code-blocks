...
// /videos, /new, /bulk, /remix, /brief/details/live and
...
const PAGES = {
  ...
  remix:              { actor: "user",    context: "submission", multi: false },
...
function buildPrompt({ account, brief, notes, checks, meta, screenshotBase, previous }) {
...
function BriefList({ workspaceId, mineKey, onList }) {
  ...
  useEffect(() => {
    ...
    onList(all.filter((b) => idsOf(b.f.accounts).includes(workspaceId) || mine.has(b.id)));
  }, [briefsQ?.data, workspaceId, mineKey, onList]);
  return null;
}
...
function Engine({ user }) {
  ...
  const mineKey = useMemo(() => idsOf(mf.briefs).join(","), [mf.briefs]);
  ...
  useEffect(() => {
    if (context === "workspace" && !briefId && urlBrief && briefs.some((b) => b.id === urlBrief)) setBriefId(urlBrief);
  }, [context, briefs, urlBrief, briefId]);
  ...
  const ownerId = isCreator ? (idsOf(brief?.users)[0] || idsOf(af?.owner)[0] || "") : "";
  ...
  const gate = useMemo(() => {
    if (!isCreator) {
      ...
      if (unwrap(mf.status) !== "Active") return { ok: false, code: "inactive", text: "Your account is inactive, so reviews are paused. Reactivate it to submit videos." };
    ...
    // Creator routes: the brief and its brand decide.
    if (context === "brief") {
    ...
    if (brief) {
      const bs = unwrap(brief.status);
      if (bs && bs !== "Active") return { ok: false, code: "closed", text: "This brief is closed to new submissions." };
    ...
    if (!workspaceId) return { ok: false, code: "noAccount", text: "This brief isn't attached to a brand workspace yet, so it can't take submissions." };
    if (!settled(afState.status)) return LOADING;
    if (ownerState.status === "success" && ownerState.f && unwrap(ownerState.f.status) !== "Active") return { ok: false, code: "inactive", text: "This brand's account is paused, so submissions are closed for now." };
    const credits = brief && hasNum(brief.videosRemainingCount) ? num(brief.videosRemainingCount) : af ? num(af.videosRemaining) : null;
    if (credits === null) return { ok: false, code: "noAccount", text: "We couldn't confirm this brand's plan. Try again in a moment." };
    if (credits <= 0) return { ok: false, code: "noCredits", text: "This brand has used every video on its plan for this billing cycle, so submissions are paused." };
    return OK;
  }, [isCreator, context, meState.status, myAccounts.length, mf.status, mf.videosRemaining, parentId, parentState.status, parent, briefId, briefLoading, brief, workspaceId, afState.status, af, ownerState]);
  ...
  const handleStart = async () => {
    ...
    if (!workspaceId || (!af && !brief)) { setError(isCreator ? "The brand's workspace is still loading. Try again in a moment." : "Choose a workspace."); return; }
    const items = buildItems();
    if (items.length === 0) { setError(tab === "url" ? "Paste at least one video link." : "Choose a video file to upload."); return; }
    for (const it of items) {
      if (it.kind === "url" && !isValidVideoUrl(it.url)) { setError(`${multi ? it.url + ": " : ""}Paste a TikTok, Instagram Reel, YouTube Short or Facebook Reel link.`); return; }
  ...
  async function runPipeline(item, myRun, ui) {
    ...
    // 4. Gemini
    const { checks, thresholdSource } = buildChecks(acct, brief);
    const meta = {
    ...
    const prompt = buildPrompt({ account: acct, brief, notes, checks, meta, screenshotBase, previous });
    ...
    const videoTitle = stripEmDash(ov.video_title || "").slice(0, 80) || (theFile?.name || handle || "Video");
    const projectIds = idsOf(brief?.projects).length ? idsOf(brief.projects) : idsOf(parent?.projects);
    const qaLabels = [...unwrapAll(acct?.qaChecklist), ...unwrapAll(brief?.qaChecklist)];
    const qaIds = [...new Set(qaLabels.map((l) => SUB_QA_IDS[l] || SUB_QA_IDS[l.replace("visability", "visibility")] || "").filter(Boolean))];
    const subFields = {
      name: videoTitle,
      ...
      creatorName: creatorName.trim() || unwrap(parent?.creatorName) || unwrap(brief?.creatorName) || "",
      creatorEmail: creatorEmail.trim() || unwrap(parent?.creatorEmail) || unwrap(brief?.creatorEmail) || "",
      submissionType: context === "submission" ? OPT.subTypeRevision : OPT.subTypeContentReview,
      ...
      originalReviewId: context === "submission" ? parentReviewId : "",
  ...
  const reset = () => { runRef.current += 1; setPage("input"); setError(""); setFailReason(""); setProcSrc(""); setResult(null); setBatch([]); };

  // ─────────────────────────────────────────────────────────────
  const title = context === "submission" ? "Submit a revision" : isCreator ? "Submit your video" : multi ? "Review videos in bulk" : "Review a video";
  const blurb = isCreator
  ...
  const gateTitle = { noAccount: isCreator ? "Not taking submissions" : "No account yet", inactive: "Account inactive", noCredits: "No videos left", closed: "Brief closed", noBrief: "Brief not found", noParent: "Video not found" }[gate.code] || "Paused";
  ...
  return (
    <div className="bl-va">
      <style>{CSS}</style>
      {user ? <UserLoader recordId={user.id} onState={setMeState} /> : null}
      ...
      {context === "workspace" && workspaceId ? <BriefList workspaceId={workspaceId} mineKey={mineKey} onList={setBriefs} /> : null}
      {workspaceId && afState.status !== "error" ? <AccountLoader key={workspaceId} recordId={workspaceId} onState={setAfState} /> : null}
      {ownerId && ownerState.status !== "error" ? <UserLoader key={`owner-${ownerId}`} recordId={ownerId} onState={setOwnerState} /> : null}
        ...
        <div className="bl-card">
          ...
          {!gate.ok && gate.code !== "loading" ? (
            <div className="bl-gate">
              ...
              {!isCreator && gate.code === "noCredits" ? <a className="bl-btn bl-primary" href="/plan">See your plan</a> : null}
            </div>
          ) : null}
            ...
            <>
              ...
              {tab === "url" ? (
                ...
                <label className="bl-drop">
                  ...
                  <span>MP4 or MOV, up to {MAX_FILE_MB}MB each{multi ? `, up to ${MAX_BULK} at a time` : ""}</span>
              ...
              <div className="bl-grid">
                <label className="bl-field"><span>{isCreator ? "Your name" : "Creator name"}{isCreator ? null : <i>optional</i>}</span><input value={creatorName} onChange={(e) => setCreatorName(e.target.value)} placeholder={unwrap(parent?.creatorName) || unwrap(brief?.creatorName) || (isCreator ? "So the brand knows who sent it" : "Who made it")} /></label>
                <label className="bl-field"><span>{isCreator ? "Your email" : "Creator email"}{isCreator ? null : <i>optional</i>}</span><input type="email" value={creatorEmail} onChange={(e) => setCreatorEmail(e.target.value)} placeholder={unwrap(parent?.creatorEmail) || unwrap(brief?.creatorEmail) || (isCreator ? "Where the result goes" : "So they get the result")} /></label>
              </div>
              <label className="bl-field"><span>{context === "submission" ? "What changed in this cut" : "Notes for the Review Agents"} <i>optional</i></span><textarea rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder={context === "submission" ? "Tell the agents what you changed since the last review" : "Anything the agents should know about this video"} /></label>

              {error ? <p className="bl-error" role="alert">{error}</p> : null}

              <div className="bl-actions">
                <span className="bl-meta">{isCreator ? `${workspaceName || "The brand"} · ${aiModeLabel} mode` : `${creditsLeft} video${creditsLeft === 1 ? "" : "s"} left this cycle · ${aiModeLabel} mode`}</span>
      ...
      {(page === "processing" || page === "done") && multi && (
        <div className="bl-card bl-proc">
          ...
          <h2>{page === "done" ? "Reviews finished" : "Review Agents watching"}</h2>
          ...
          <div className="bl-list">
            {batch.map((r) => (
              <div className="bl-row" key={r.key}>
                ...
                {r.status === "failed" ? <span className="bl-chip bl-bad" title={r.error}>Didn't complete</span> : null}
              </div>
            ))}
          </div>
          {page === "done" ? (
      ...
      {page === "done" && !multi && result && (
        <div className="bl-card bl-proc">
          ...
          <a className="bl-btn bl-primary" href={detailHref(result.subId)}>{isCreator ? "See your result" : "Open the review"}<ArrowRight size={14} /></a>
        </div>
      )}
...
.bl-quote{font-size:13px;color:var(--navy);background:rgba(135,156,247,.12);border-left:3px solid var(--peri);border-radius:8px;padding:10px 12px;line-height:1.5;white-space:pre-wrap}
.bl-list{display:flex;flex-direction:column;gap:8px;width:100%;text-align:left}
.bl-row{display:flex;align-items:center;gap:10px;background:#fff;border:1px solid rgba(0,0,0,.06);border-radius:10px;padding:10px 12px;font-size:13px}
...
.bl-chip.bl-run{background:rgba(135,156,247,.18);color:var(--peri3)}
