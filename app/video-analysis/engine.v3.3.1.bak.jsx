...
function Engine({ user }) {
  ...
  const reset = () => { runRef.current += 1; setStep(0); setCtxChoice(""); setPdf(null); setAgents([]); setKind(""); setAddCreator(false); setPage("input"); setError(""); setFailReason(""); setProcSrc(""); setResult(null); setBatch([]); };
  ...
  const blurb = STEPPER
    ? `Review, analyse or remix a video. Everything is filed under ${workspaceName || "your workspace"}.`
    : isCreator
  ...
  const order = !kind ? ["kind"]
    : kind === "review" ? ["kind", manyWs ? "workspace" : null, "upload", "context", ctxChoice ? "detail" : null, needsQa ? "qa" : null].filter(Boolean)
    : ["kind", manyWs ? "workspace" : null, "source", "notes"].filter(Boolean);
  const KIND_COPY = {
            ...
            <>
              ...
              {stepKey === "kind" ? (
                <div className="bl-step">
                  <label className="bl-field"><span>What do you want to do?</span></label>
                  <div className="bl-tabs" role="radiogroup" aria-label="What do you want to do">
                    <button type="button" role="radio" aria-selected={kind === "review"} onClick={() => pickKind("review")}>Review</button>
                ...
                <div className="bl-step">
                  ...
                  {tab === "url" ? (
                    <label className="bl-field">
                      ...
                      <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://www.tiktok.com/@creator/video/..." />
                    </label>
                  ) : (
                    <label className="bl-drop" onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); const list = Array.from(e.dataTransfer?.files || []); if (list.length) { setFile(list[0]); setError(""); } }}>
                      <Upload size={18} />
              ...
              <div className="bl-actions">
                <span className="bl-meta">{isLast && kind ? `${creditsLeft} video${creditsLeft === 1 ? "" : "s"} left this cycle · ${aiModeLabel} mode` : ""}</span>
                <span className="bl-foot">
                  ...
                  <button
                    ...
                    disabled={(stepKey === "kind" && !kind) || (stepKey === "workspace" && !workspaceId) || (stepKey === "upload" && !file) || (stepKey === "source" && (tab === "url" ? !url.trim() : !file)) || (stepKey === "context" && !ctxChoice && !isLast) || (stepKey === "qa" && agents.length === 0)}
                    onClick={() => {
                      setError("");
                      ...
                      if (stepKey === "source" && tab === "url" && !isValidVideoUrl(url.trim())) { setError("Paste a TikTok, Instagram Reel, YouTube Short or Facebook Reel link."); return; }
