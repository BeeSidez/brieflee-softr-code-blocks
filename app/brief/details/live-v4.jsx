...
const SECTION_PART_KEYS = {
  offer: ["deal", "fee", "commission", "gift"],
...
const SECTION_META = {
  ...
  offer: {
    ...
    partLabels: { fee: "Fee", commission: "Commission", gift: "Gift" },
    partFields: { deal: "offer", fee: "offer_fee", commission: "offer_commission", gift: "offer_gift" }
  },
  audienceCustomer: {
  ...
  usageRights: {
    name: "Usage rights", icon: ICON_RIGHTS, tip: "Who can use the content, where, and for how long.",
...
export default function Block() {
  ...
  // Hero image chain — mirrors /briefs/details, minus the favicon
  // (Google s2 returns 128px which blurs at hero size). Falls to the
  // Brieflee mascot before favicon ever gets reached.
  const heroImage = extractThumbUrl(f.thumbnail)              // 1. brief's own upload
    || (typeof f.image_url === "string" && f.image_url)        // 2. AI-scraped product image
    || extractThumbUrl(f.project_thumbnail)                    // 3. project's thumbnail
    || extractThumbUrl(f.logo_file)                            // 4. account's uploaded logo
    || BRIEFLEE_MASCOT;                                        // 5. fallback
  // Resolved ONCE. The gate below and the Product card must agree:
  // a looser gate than the render uses would open an empty card for
  // a product carrying only the placeholder.
        ...
        <div className="bl-live-inner container mx-auto px-4 w-full">
          ...
          <header className="bl-live-hero">
            ...
            <div className="bl-live-hero-right">
              <img src={heroImage} alt={briefName} className="bl-live-hero-image" draggable={false} />
              {typeof f.product_url === "string" && f.product_url.trim() ? (
                <a className="bl-live-hero-caption" href={f.product_url.trim()} target="_blank" rel="noreferrer">View the product</a>
              ) : null}
            </div>
          </header>
            ...
            <>
              <nav className="bl-live-nav" aria-label="On this page">
                {model.order.filter(sectionHasAnyContent).map((k) => (
                  <button key={k} type="button" className={openSecs[k] ? "is-on" : ""} onClick={() => scrollToSection(k)}>{SECTION_META[k].name}</button>
                ))}
              </nav>
          ...
          <footer className="bl-live-footer">
            <div className={`bl-live-footer-card ${isClosed ? "is-closed" : ""}`}>
              <div className="bl-live-footer-text">
                ...
                {!isClosed && (platforms.length || aspectRatios.length || contentDelivery || closeDate) ? (
                  <p className="bl-live-footer-meta">
                    {[platforms.join(", "), aspectRatios.join(", "), contentDelivery, closeDate ? `Closes ${closeDate}` : ""].filter(Boolean).join(" · ")}
                  </p>
                ) : null}
              </div>
              <button
...
function BentoCard({ title, tooltip, wide, full, defaultOpen = false, copyText, id, open: openProp, onToggle, children }) {
  ...
  return (
    <section id={id} className={`bl-live-bento-card ${spanClass} ${open ? "is-open" : ""}`}>
      <button
        type="button"
        onClick={toggle}
        aria-expanded={open}
      ...
      >
        ...
        <ChevronDown size={16} style={{ color: open ? PERIWINKLE_HOVER : NAVY, transform: open ? "rotate(0)" : "rotate(-90deg)", transition: "transform 0.2s ease, color 0.2s ease" }} />
      </button>
      {open && (
...
function Style() {
  return (
    <style>{`
      ...
      .bl-live-hero-caption {
        display: inline-block; margin-top: 8px;
        font-size: 12px; font-weight: 500; color: ${PERIWINKLE_HOVER}; text-decoration: none;
      }
      .bl-live-hero-caption:hover { text-decoration: underline; }
      ...
      .bl-live-bento-card.is-open { border-color: ${PERIWINKLE}; }
      .bl-live-bento-head {
      ...
      .bl-live-bento-body { font-size: 13.5px; line-height: 1.6; color: ${NAVY_DEEP}; }
      .bl-live-bento-body p, .bl-live-bento-body li { max-width: 68ch; }
      ...
      .bl-live-footer-meta {
        ...
        font-size: 12px !important; color: ${PERIWINKLE_HOVER} !important; font-weight: 500;
      }
      ...
      .bl-live-nav button.is-on { background: ${PERIWINKLE}; border-color: ${PERIWINKLE}; color: #FFFFFF; }
      .bl-live-nav button.is-on:hover { background: ${PERIWINKLE_HOVER}; border-color: ${PERIWINKLE_HOVER}; color: #FFFFFF; }
      .bl-live-nav button {
        padding: 8px 14px;
        background: ${TINT_BG};
        border: 1px solid ${TINT_BORDER};
      ...
      .bl-live-sticky {
        ...
        border-top: 1px solid ${TINT_BORDER};
      ...
      @media (max-width: 880px) {
        ...
        .bl-live-hero-right { display: flex; flex-direction: column; align-items: flex-start; }
        .bl-live-hero-image { aspect-ratio: 1 / 1; width: 200px; }
        ...
        .bl-live-nav {
          ...
          margin: 0 -16px; padding: 8px 16px; border-radius: 0; top: 0;
        }
